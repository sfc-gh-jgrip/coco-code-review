...
class PythonFanoutOrchestrator(Orchestrator):
    ...
    async def _run_inner(
        ...
        return RunResult(
            ...
            stats=PipelineStats(
                reviewer_names=[r.name for r in active_reviewers],
                replicas_dispatched=reviewer_replica_count,
                replicas_succeeded=reviewer_success_count,
                replicas_failed=reviewer_exception_count,
                raw_candidates=candidate_count,
                deduped_candidates=deduped_count,
                verified=len(verified_findings),
                dropped_verifier_error=dropped_verifier_error,
